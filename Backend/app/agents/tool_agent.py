...
from app.tools.news_tools import extract_news_query
...
async def tool_agent_node(state: AgentState) -> Dict[str, Any]:
    ...
    tools_to_run = []
    if any(k in query for k in ["news", "headline", "gnews", "article", "breaking news"]):
        topic = extract_news_query(state.get("user_query", ""))
        tools_to_run.append({"name": "get_news", "args": {"query": topic, "max_results": 10}})
    ...
    if not tools_to_run:
        app_logger.info("[ToolAgent] No matching tools to execute")
        return {
            "current_agent": "tool_agent",
            "next_agent": "validator",
            "tool_results": [],
            "intermediate_reasoning": state.get("intermediate_reasoning", []) + ["Tool Agent found no matching tools"]
        }
